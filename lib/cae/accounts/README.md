...
```elixir
{:ok, result} = Cae.Accounts.register_student(
  %{
...
```elixir
user = Cae.Accounts.get_user_by(%{email: "juan@uni.edu"})
profile = Cae.Accounts.get_student_profile(user.id)
```
...
```elixir
psychologists = Cae.Accounts.list_users_by_role("psychologist")

{:ok, prof} = Cae.Accounts.create_professional(%{
...
```elixir
user = Cae.Accounts.get_user!(123)
{:ok, deactivated} = Cae.Accounts.deactivate_user(user)
{:ok, promoted} = Cae.Accounts.promote_to_admin(deactivated)
```
...
**Ejecutar tests:**
```bash
mix test test/cae/accounts_test.exs
```
